...
void PrintScriptHeader ( PrintScript_t *ps )
{
	...
	switch (script_fform)
	{
	    ...
	    case FF_SH:
	       ps->add_index = script_array > 0;
	       break;

	    case FF_PHP:
		if ( script_array > 0 )
		    fprintf(ps->fo->f,"$%s = array();\n\n",script_varname);
		break;

	    case FF_MAKEDOC:
		if ( script_array > 0 )
		    fprintf(ps->fo->f,"%s = @LIST\n\n",script_varname);
		break;
...
void PrintScriptFooter ( PrintScript_t *ps )
{
	...
	switch (script_fform)
	{
	    ...
	    case FF_SH:
		if (script_array)
		    fprintf(ps->fo->f,"%s_N=%u\n\n",script_varname,ps->index);
		break;
