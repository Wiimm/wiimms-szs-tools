...
static enumError cmd_patch()
{
	...
	const bool dest_is_source = !strcmp(dest,param->arg);
...
#endif
{
    ...
    SetupLib(argc,argv,WCTCT_SHORT);
    ctcode_enabled = true;
    ...
    enumError err = CheckEnvOptions("WCTCT_OPT",CheckOptions);
    if (err)
	hint_exit(err);
