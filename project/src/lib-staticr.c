...
static enumError ReadCodeFile ( staticr_t *str, ccp fname, u8 **data, uint *size )
{
    DASSERT(str);
    DASSERT(fname);
    DASSERT(data);
...
//	else if (!strcmp(fname,"@XXX"))		wcm = GetXXXX(str->mode);
...
enumError AddWCodeFile ( staticr_t *str, ccp fname )
{
    ...
    u8 *data;
    uint size;
    enumError err = ReadCodeFile(str,fname,&data,&size);
    if (err)
	return err;
...
)
{
    ...
    u8 *gh_data = 0;
    uint gh_size = 0;
    enumError err = ReadCodeFile(str,fname,&gh_data,&gh_size);
    if (err)
	return err;
