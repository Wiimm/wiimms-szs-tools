...
static enumError cmd_copy()
{
	...
	char dest[PATH_MAX];
	SubstDest(dest,sizeof(dest),param->arg,opt_dest,"\1P/\1N\1?T",
			GetExtFF(dest_ff,0),false);
	if ( verbose >= 0 || testmode )
	{
...
static enumError cmd_cff()
{
	...
	if (!dest)
	{
	    SubstDest(dest_buf,sizeof(dest_buf),param->arg,opt_dest,
...
static void analyze_kcl ( kcl_t *kcl )
{
	...
	float *f = td->normal->v;
	uint nf = sizeof(td->normal)/sizeof(float);
	for ( ; nf > 0; nf--, f++ )
	{
	    if ( n_min > *f ) n_min = *f;
	    if ( n_max < *f ) n_max = *f;
...
#endif
{
    ...
    enumError err = CheckEnvOptions("WKCLT_OPT",CheckOptions);
    if (err)
	hint_exit(err);
